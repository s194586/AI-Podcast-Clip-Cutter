...
class ClipEvaluation(Base):
    ...
    # Historical heuristic fields. Gemini boundary decisions leave these NULL
    # unless a compatibility caller explicitly supplies a legacy value.
    ...
    payoff_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    boundary_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    privacy_risk: Mapped[str | None] = mapped_column(String(32), index=True, nullable=True)
    ...
    crop_advice: Mapped[str | None] = mapped_column(String(64), nullable=True)
    needs_more_context: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
