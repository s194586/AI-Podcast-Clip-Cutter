...
describe('Product UI v0.5', () => {
  ...
  it('Jump to Start uses the latest edited_start', async () => {
  ...
  it('Reject persists and keeps the rejected status visible', async () => {
    ...
    expect(screen.getByText('Change the clip decision before rendering this rejected clip.')).toBeInTheDocument()
  })
  ...
  it('shows render success', async () => {
    mockApi([
      ...editorRoutes([clip()]),
      { method: 'POST', path: '/projects/3/render', json: { status: 'completed', clip_id: 'clip_001', start: 10.5, end: 18.5, duration: 8, warnings: [], clip: clip({ render_status: 'completed' }) } },
    ])
