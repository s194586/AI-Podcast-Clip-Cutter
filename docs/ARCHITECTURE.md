...
Gemini calls use a configured per-attempt HTTP timeout (`GEMINI_REQUEST_TIMEOUT_SECONDS`, default 300 seconds), one SDK attempt, and a killable child-process deadline. The child is a dedicated `python -m apps.review_agent.gemini_worker` subprocess rather than a `multiprocessing` bootstrap, and exchanges a versioned JSON request/response over standard-input/output pipes. The Gemini client is created inside that child; timeout or local cancellation escalates from terminate to kill and always reaps the process. The project batch has a separate deadline (`GEMINI_BATCH_TIMEOUT_SECONDS`, default 1800 seconds). Review emits safe per-clip events and coarse progress from 85 through 95 percent. HTTP 499 is treated as a controlled upstream-cancelled request for that clip, not success and not an infinite retry.
