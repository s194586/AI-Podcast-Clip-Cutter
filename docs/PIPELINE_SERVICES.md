...
`GEMINI_REQUEST_TIMEOUT_SECONDS` defaults to 300 seconds. It configures the installed `google-genai` client's millisecond HTTP timeout, explicitly limits SDK attempts, and bounds the same call in the dedicated `python -m apps.review_agent.gemini_worker` subprocess so a blocked client cannot leave an abandoned thread. The full request travels through stdin rather than process arguments, the versioned result is validated from stdout, and timeout/cancellation uses terminate-then-kill cleanup followed by an unconditional reap. Corrective retries use the same remaining deadline. `GEMINI_BATCH_TIMEOUT_SECONDS` defaults to 1800 seconds and bounds the complete clip loop.
