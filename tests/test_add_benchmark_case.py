...
class AddBenchmarkCaseTests(unittest.TestCase):
    ...
    def test_creates_new_podcast_case_and_copies_video(self):
        base, cases_path, assets_root = self._workspace()
        source = base / "sample.mp4"
        source.write_bytes(b"video")

        result = add_benchmark_case.add_case(
            case_id="my_podcast_01",
            video_path=str(source),
            content_type="podcast",
            review_batch="podcast_only_v1",
            notes="new podcast test",
            cases_path=cases_path,
            assets_root=assets_root,
        )

        copied = assets_root / "my_podcast_01" / "input" / "source.mp4"
        self.assertTrue(copied.exists())
        payload = json.loads(cases_path.read_text(encoding="utf-8"))
        self.assertEqual(payload["cases"][0]["id"], "my_podcast_01")
        self.assertEqual(payload["cases"][0]["expected_content_type"], "podcast")
        self.assertEqual(payload["cases"][0]["review_batch"], "podcast_only_v1")
        self.assertFalse(payload["cases"][0]["include_generic_baseline"])
    ...
    def test_does_not_overwrite_existing_case_without_force(self):
        ...
        payload = {"cases": [{"id": "my_case", "expected_content_type": "podcast"}]}
    ...
    def test_force_overwrites_existing_case_as_podcast(self):
        base, cases_path, assets_root = self._workspace()
        source = base / "sample.mp4"
        source.write_bytes(b"video")
        cases_path.write_text(
            json.dumps({"cases": [{"id": "my_case", "expected_content_type": "podcast"}]}),
            encoding="utf-8",
        )
        ...
        self.assertEqual(payload["cases"][0]["expected_content_type"], "podcast")
        self.assertEqual(payload["cases"][0]["review_batch"], "podcast_only_v2")

    def test_rejects_legacy_content_type(self):
        _base, cases_path, assets_root = self._workspace()
...
class RunLocalBenchmarkTests(unittest.TestCase):
    def test_builds_local_only_benchmark_command(self):
        command = run_local_benchmark.build_benchmark_command(review_batch="podcast_only_v1", extra_args=["--top", "3"])
        ...
        self.assertIn("podcast_only_v1", command)
